...
@onready var ClarasA = preload("res://Mundo/polillas/Claras/ClarasA.tscn")
@onready var MelanicasA = preload("res://Mundo/polillas/MelanicasA/MelanicasA.tscn")

#CARGAR PANTALLA DE MUERTE
@onready var PantallaDeMuerte = preload("res://Mundo/PantallaDeMuerte/PantallaDeMuerte.tscn")
...
# Variable para la animacion de la pantalla de muerte
...
func _PantallaDeMuerte(death_scene):
	...
	add_child(pantallaMuerte)
	var animacionMuerte = pantallaMuerte.get_node("AnimationPlayer")
	animacionMuerte.play("PantallaDeMuerte")
	await animacionMuerte.animation_finished
	LoadManager.load_scene("res://Mundo/PantallaInicio/pantalla_inicio.tscn")
	
func _on_Claras_pressed():
	print("Polilla capturada")
	Conteo.G_claras -= 1
	puntaje_claras += 1
	Conteo.Consumidas += 1
	get_node("MarginContainer/VBoxContainer/Cla").text = "CLARAS: " + str(puntaje_claras)

func _on_Melanicas_pressed():
	Conteo.G_melanicas -= 1
	puntaje_melanicas += 1
	Conteo.Consumidas += 1
